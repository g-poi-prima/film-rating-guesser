...
export async function getProfile(): Promise<User & { totalScore: number; gamesPlayed: number }> { const { data } = await api.get('/profile'); return data; }
export async function updateProfile(params: { username?: string; email?: string; password?: string; avatar?: string }): Promise<User> { const { data } = await api.put('/profile', params); return data; }